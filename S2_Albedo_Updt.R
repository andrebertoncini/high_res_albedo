...
for (j in 6:20) {
  ...
  #Uncomment below to run the algorithm with a specific number of cluster classes
  #j = 16 
  
  #Perform kmeans classification based on Sentinel-2 surface reflectance bands
  
  setwd("/path to blue-sky albedo outputs")
  ...
  kmncluster <- kmeans(na.omit(nr), centers = j, iter.max = 1000, nstart = 10, algorithm = "Lloyd")
  ...
  #Input Albedo-to-Nadir ratios at MODIS resolution
  ...
  for (l in 1:6) {
    ...
    write.table(ratio_table, file = paste0("/path to blue-sky albedo outputs/Ratios_", img_date, "_bsa_B", band, ".csv"), sep = ";", dec = ".")
    ...
    write.table(ratio_table, file = paste0("/path to blue-sky albedo outputs/Ratios_", img_date, "_wsa_B", band, ".csv"), sep = ";", dec = ".")
  ...
  #Calculate directional blue-sky albedo conditionally based on the presence of snow
  #Narrow to broadband conversion follows Li et al. (2018) "Preliminary assessment of 20-m surface albedo retrievals 
  #from sentinel-2A surface reflectance and MODIS/VIIRS surface anisotropy measures" - Remote Sensing of Environment.
  ...
  #f_dif from Ellis and Pomeroy (2007) "Estimating sub-canopy shortwave irradiance to melting snow on forested slopes"
  ...
  #This part is from Klein et al. (1998) "Improving snow cover mapping in forests through the use of a canopy 
  #reflectance model" - Hydrological Processes
