...
for (w in c(1,2,3,4,6,7)) {
  ...
  for (j in 1:length(theta_s_vec)) {
    ...
    if(sum(is.na(theta_s)) >= 13 | sum(is.na(ifelse(ref < 0, NA, ref))) >= 13) {
      ...
      A <- matrix(c(rep(1,length(b)), na.omit(k_vol_omega), na.omit(k_geo_omega)), ncol = 3, nrow = length(b))
      ...
      x <- lsqlincon(C = W %*% A, d = W %*% matrix(b), lb = c(0,0,0))
      ...
      U_wdr <- c(1, k_vol_omega, k_geo_omega)*mean_weights[9]
      ...
      U_wsa <- c(1, k_vol_l, k_geo_l)*mean_weights[9]
